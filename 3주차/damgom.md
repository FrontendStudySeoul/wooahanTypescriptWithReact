...
> fetch API의 `json()` 메서드는 any 타입을 반환한다.
...
### any 타입과 유사한 unknown 타입이 추가된 이유
- `unknown` 타입 변수에 할당할 때는 컴파일러 아무런 경고를 주지 않지만
- 함수 뿐만 아니라 객체 내부에 접근하는 모든 시도에서 에러가 발생한다
- `unknown` 타입은 어떤 타입이 할당될 수 없음을 나타내기 때문에 `unknown` 타입으로 할당된 변수는 어떤 값이든 올 수 있음을 의미하는 동시에 개발자에게 엄격한 타입 검사를 강제하는 의도를 담고 있다.
...
#### 3. 예상할 수 없는 데이터
    - 타입스크립트 4.4부터 `try-catch`` 에러 타입이 any에서 `unknown`으로 변경되어서 에러 핸들링할 때도 unknown을 사용
...
#### AS-IS
<img src="https://github.com/FrontendStudySeoul/wooahanTypescriptWithReact/assets/80238096/0b1891e8-4480-4dec-9505-44dfbc73d548" width="600px" />


#### TO-BE
<img src="https://github.com/FrontendStudySeoul/wooahanTypescriptWithReact/assets/80238096/eecbf199-5325-47b4-a8fc-03dbe56721c9" width="600px" />
...
- never 타입은 값을 **반환할 수 없는 타입**을 말한다
...
### 튜플
- 대괄호를 사용해 선언할 수 있고 **원소의 타입과 개수를 제한**할 수 있다
...
- `useState`는 튜플 타입을 반환한다.
- `useState`는 배열의 원소의 자리마다 명확한 의미를 부여하기 때문에 튜플 타입을 사용
...
<img width="600" src="https://github.com/FrontendStudySeoul/wooahanTypescriptWithReact/assets/80238096/cb4cf8ca-4af1-4d05-bf82-459b5933e8dd">

#### TO-BE
<img width="600" src="https://github.com/FrontendStudySeoul/wooahanTypescriptWithReact/assets/80238096/cf156ee4-0e99-481f-b73f-99e6caef9f4a">
...
![image](https://github.com/FrontendStudySeoul/wooahanTypescriptWithReact/assets/80238096/0076802a-2375-4335-b16b-9cd3cf3fa166)
...
> Vscode에서 Intersection한 타입 깔끔하게 보기
``` ts
type Prettify<T> = {
  [K in keyof T]: T[K];
} & {}
